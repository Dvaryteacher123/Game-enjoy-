...
// -------------------- Mara kwa mara --------------------
const ADMIN_EMAIL = "Dullamanyama0@gmail.com"; // barua pepe inayoruhusiwa kuandika
...
// Sikiliza mabadiliko ya hali ya kuingia (kwa matumizi katika HTML)
export const onAuthStateChangedListener = (callback) => {
  return onAuthStateChanged(auth, callback);
};

// (Toleo rahisi la kutumia) 
export const onAuthStateChanged = onAuthStateChangedListener;

// -------------------- Kazi za Usalama --------------------
// Angalia kama mtumiaji aliyeingia ndiye admin anayeruhusiwa
export const isAdminUser = (user) => {
  if (!user) return false;
  return user.email === ADMIN_EMAIL;
};

// -------------------- Kazi za Firestore (CRUD) --------------------
// Pata michezo yote (iliyopangwa kwa createdAt)
export const getGames = async () => {
  try {
    const gamesRef = collection(db, "games");
    // Pangilia kwa tarehe (mpya kwanza)
...
// Ongeza mchezo (inakagua kama mtumiaji ni admin)
export const addGame = async (gameData) => {
  const user = auth.currentUser;
  if (!user) throw new Error("Hujuingia. Tafadhali ingia kama admin.");
...
// Futa mchezo (inakagua kama mtumiaji ni admin)
